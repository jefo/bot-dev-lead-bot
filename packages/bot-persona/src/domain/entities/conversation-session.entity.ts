...
export const ConversationSession = createEntity({
  ...
  actions: {
    transitionTo(state: ConversationSessionState, newStateId: string): ConversationSessionState {
      if (state.status !== 'active') throw new Error('Cannot transition in a non-active session.');
      return { ...state, currentStateId: newStateId, updatedAt: new Date() };
    },
    updateData(state: ConversationSessionState, dataToMerge: Record<string, any>): ConversationSessionState {
      return {
        ...state,
