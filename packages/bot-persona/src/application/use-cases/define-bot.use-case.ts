...
import { DefineBotCommand, DefineBotCommandSchema } from '../dtos/define-bot.dto';
import { saveBotPort, saveConversationPort } from '../ports/bot-definition.ports';

/**
 * Use Case для определения нового бота и его логики.
 */
export async function defineBotUseCase(command: unknown): Promise<{ botId: string }> {
  // 1. Валидация базовой структуры команды
  const { botName, fsmDefinition, viewModelDefinition } = DefineBotCommandSchema.parse(command);
