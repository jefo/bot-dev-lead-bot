...
import { findBotPersonaByIdPort } from "../../domain/ports";
...
describe("FAQ Bot Integration Test", () => {
  ...
  it("should create FAQ bot persona and handle conversation flow", async () => {
        ...
        nodes: [
          {
            ...
            props: { 
              ...
              options: ["FAQ1 - Как работает этот бот?", "FAQ2 - Где найти документацию?", "FAQ3 - Как получить помощь?", "END - Завершить"]
            }
          },
          {
            id: "faq1",
            ...
            props: { 
              ...
              answer: "Этот бот использует конечный автомат для управления диалогом. Каждое состояние соответствует определенному этапу взаимодействия."
            }
          },
          {
            id: "faq2",
          ...
          },
          {
            ...
            props: { 
              ...
              answer: "Вы можете получить помощь, создав issue в репозитории проекта или обратившись в службу поддержки."
            }
          },
          {
            id: "end",
            component: "GoodbyeMessage",
            props: { text: "Спасибо за использование нашего бота! До свидания!" }
          }
        ]
      }
    };
    ...
    const { inMemoryFindAllBotPersonasAdapter } = await import("../../../src/composition");
  ...
  it("should handle invalid input gracefully", async () => {
    ...
    for (const [id, persona] of botPersonas) {
      console.log("Found persona (test 2):", persona.state.name, "with id:", id);
