import type {
	PathSelectedOutput,
	UserWelcomedOutput,
} from "../../application/dtos/onboarding.dtos";
 ...
 */
export async function welcomeUserAdapter(
	output: UserWelcomedOutput,
): Promise<void> {
	const text =
		"Здравствуйте! Увидел ваш интерес на Kwork. Я — бот-ассистент, который поможет за 3 минуты собрать из вашей идеи четкое ТЗ для разработчика. Готовы?";
	const keyboard = {
		inline_keyboard: [
 ...
 */
export async function pathSelectedAdapter(
	output: PathSelectedOutput,
): Promise<void> {
